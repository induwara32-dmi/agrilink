...
function DeliveryPanel({ delivery, canManage, onUpdated }: { delivery: DeliveryTracking; canManage: boolean; onUpdated: () => void }) {
  ...
  const transporter = delivery.transportJob?.transporter ?? null;
  const transporterName = transporter ? (transporter.user.profile ? `${transporter.user.profile.firstName} ${transporter.user.profile.lastName}` : transporter.businessName ?? 'Assigned platform transporter') : null;
...
function MethodDetails({ delivery }: { delivery: DeliveryTracking }) {
  ...
  if (delivery.method === 'PLATFORM_TRANSPORTER') { const transporter = delivery.transportJob?.transporter; const name = transporter ? (transporter.user.profile ? `${transporter.user.profile.firstName} ${transporter.user.profile.lastName}` : transporter.businessName ?? 'Assigned platform transporter') : 'Assignment pending'; return <div className="space-y-2 rounded-2xl border border-border bg-slate-50 p-4 text-sm text-slate-600"><p className="font-semibold text-slate-900">Platform transporter</p><p>{name}</p><p>{delivery.transportJob?.vehicle ? `${delivery.transportJob.vehicle.registrationNumber} · ${delivery.transportJob.vehicle.type.replaceAll('_', ' ')}` : 'Vehicle pending'}</p></div>; }
