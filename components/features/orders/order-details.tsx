...
import { CheckCircle2, Download, MessageCircle, Package, Truck } from 'lucide-react';
import { OrderStatusBadge } from './order-status-badge';
import { FarmerOrderStatusBadge } from './farmer-order-status-badge';
...
import { acceptFarmerOrder, cancelOrder, getOrder } from '@/lib/api/commerce';
...
export function OrderDetails({ orderId }: { orderId: string }) {
  ...
  const accept = useMutation({ mutationFn: (farmerOrderId: string) => acceptFarmerOrder(orderId, farmerOrderId), onSuccess: response => { client.setQueryData(['orders', orderId], response); void client.invalidateQueries({ queryKey: ['orders'] }); setToast({ message: 'Order accepted. The transporter assignment has been triggered.' }); }, onError: error => setToast({ message: error instanceof ApiClientError ? error.message : 'Unable to accept this order.', tone: 'error' }) });
  ...
  {item.farmerOrders.map(group => { const canAccept = user?.role === 'FARMER' && user.id === group.farmer.userId && group.status === 'PENDING'; return <Card key={group.id} className="border-border/80 bg-white"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><CardTitle>{group.farmer.farmName} · {group.farmerOrderNumber}</CardTitle><FarmerOrderStatusBadge status={group.status} /></div></CardHeader><CardContent className="space-y-3">{group.items.map(orderItem => <div key={orderItem.id} className="flex items-center justify-between rounded-2xl border border-border bg-slate-50 p-3 text-sm"><div><p className="font-semibold text-slate-900">{orderItem.productName}</p><p className="text-slate-600">Qty {orderItem.quantity} {orderItem.unit} · {money(orderItem.unitPrice, item.currency)}</p></div><p className="font-semibold text-slate-900">{money(orderItem.lineTotal, item.currency)}</p></div>)}{canAccept ? <Button disabled={accept.isPending} onClick={() => accept.mutate(group.id)}><CheckCircle2 className="mr-2 h-4 w-4" /> {accept.isPending ? 'Accepting…' : 'Accept order'}</Button> : null}</CardContent></Card>; })}</div>
  <aside className="space-y-6"><Card className="border-border/80 bg-white"><CardHeader><CardTitle>Payment summary</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-slate-600"><div className="flex justify-between"><span>Subtotal</span><span>{money(item.subtotal, item.currency)}</span></div><div className="flex justify-between"><span>Delivery</span><span>{money(item.deliveryTotal, item.currency)}</span></div><div className="flex justify-between"><span>Discount</span><span>-{money(item.discountTotal, item.currency)}</span></div><div className="flex justify-between border-t border-border pt-3 text-base font-semibold text-slate-900"><span>Total</span><span>{money(item.grandTotal, item.currency)}</span></div></CardContent></Card>
