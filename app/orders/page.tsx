...
function OrdersContent() {
  ...
  {orders.isLoading ? <LoadingSkeleton /> : orders.isError ? <ErrorState title="Orders unavailable" description="We could not load your order history." onRetry={() => void orders.refetch()} /> : items.length ? <div className="space-y-4">{items.map(order => <OrderCard key={order.id} order={order} />)}</div> : <EmptyState icon={<Package />} title="No orders found" description="Try changing the search or status filter." />}
