...
import { Bell, CheckCircle2, ShoppingBasket, ShoppingCart, Truck, Wallet } from 'lucide-react';
...
export function BuyerDashboardPage() {
  ...
  return <div className="space-y-6"><section className="relative overflow-hidden rounded-[2rem] border border-border bg-gradient-to-br from-primary to-secondary p-6 text-white shadow-sm">
    <ShoppingBasket className="pointer-events-none absolute -bottom-10 -right-8 h-48 w-48 text-white/10" strokeWidth={1} aria-hidden="true" />
    <div className="relative z-10"><p className="text-sm font-semibold uppercase tracking-[0.3em] text-white/80">Buyer analytics</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Welcome back, {user?.profile?.firstName ?? 'Buyer'}</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-white/85">Review your orders, spending, deliveries, and marketplace preferences.</p></div>
  ...
  {!enabled ? <EmptyState title="Select a custom range" description="Choose both a start and end date to load analytics." /> : analytics.isLoading ? <LoadingSkeleton /> : analytics.isError ? <ErrorState title="Buyer analytics unavailable" description="We could not load your dashboard summary." onRetry={() => void analytics.refetch()} /> : report ? <>
    ...
    <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]"><BuyerRecentOrders /><Card className="border-border/80 bg-white"><CardHeader><CardTitle>Upcoming Deliveries</CardTitle></CardHeader><CardContent className="p-0">{deliveries.isLoading ? <div className="p-6"><LoadingSkeleton /></div> : deliveries.isError ? <div className="p-6"><ErrorState title="Deliveries unavailable" description="Upcoming deliveries could not be loaded." onRetry={() => void deliveries.refetch()} /></div> : deliveries.data?.data.filter(item => !['DELIVERED', 'CANCELLED', 'FAILED'].includes(item.status)).length ? <ClickableRowList>{deliveries.data.data.filter(item => !['DELIVERED', 'CANCELLED', 'FAILED'].includes(item.status)).map(item => <ClickableRow key={item.id} href={`/orders/${item.farmerOrder.order.id}/tracking`}>
      <p className="font-semibold text-slate-900">{item.farmerOrder.farmer.farmName}</p>
      <span className="text-slate-600">{item.status.replaceAll('_', ' ')} · {item.estimatedDeliveryAt ? new Date(item.estimatedDeliveryAt).toLocaleString() : 'Scheduling pending'}</span>
    </ClickableRow>)}</ClickableRowList> : <div className="p-6"><EmptyState icon={<Truck />} title="No upcoming deliveries" description="Active deliveries will appear here." /></div>}</CardContent></Card></section>
