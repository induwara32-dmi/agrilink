...
export type RemovedCartItem = { id: string; productId: string; productName: string };
export type Cart = { id: string; groups: CartGroup[]; savedForLater: CartItem[]; currency: string | null; subtotal: string; removedItems: RemovedCartItem[] };
...
export type FarmerOrderStatusCode = 'PENDING' | 'ACCEPTED' | 'PREPARING' | 'READY_FOR_PICKUP' | 'IN_TRANSIT' | 'DELIVERED' | 'REJECTED' | 'CANCELLED';
...
export const acceptFarmerOrder = (orderId: string, farmerOrderId: string) => apiRequest<CheckoutOrder>(`/orders/${encodeURIComponent(orderId)}/farmer-groups/${encodeURIComponent(farmerOrderId)}/accept`, { method: 'POST', authenticated: true });
