...
export class CommerceService extends BaseService {
  ...
  private groupCart<T extends { items: Array<{ product: { farmer: { id: string; farmName: string } }; savedForLater: boolean }> }>(cart: T) {
    const groups = new Map<string, { farmer: { id: string; farmName: string }; items: T['items'] }>();
    ...
    for (const item of cart.items) {
      ...
      if (existing) existing.items.push(item); else groups.set(item.product.farmer.id, { farmer: item.product.farmer, items: [item] as T['items'] });
    }
    return { ...cart, items: undefined, groups: [...groups.values()], savedForLater };
  }
