...
export class CatalogService extends BaseService {
  ...
  public async createCategory(input: CategoryInput, actor: Actor) { if (input.parentId && !await this.repository.findCategory(input.parentId)) throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'PARENT_CATEGORY_NOT_FOUND', 'Parent category not found.'); if (await this.repository.findCategoryByName(input.name)) throw new ApiError(HTTP_STATUS.CONFLICT, 'CATEGORY_NAME_EXISTS', 'A category already uses this name.'); try { return await this.repository.createCategory({ ...input, slug: input.slug ?? slugify(input.name) }, actor.userId, actor.requestId); } catch (error) { this.translateConflict(error); } }
  public async updateCategory(id: string, input: CategoryUpdateInput, actor: Actor) { if (!await this.repository.findCategory(id)) throw new ApiError(HTTP_STATUS.NOT_FOUND, 'CATEGORY_NOT_FOUND', 'Category not found.'); if (input.parentId === id) throw new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_CATEGORY_PARENT', 'A category cannot be its own parent.'); if (input.name && await this.repository.findCategoryByName(input.name, id)) throw new ApiError(HTTP_STATUS.CONFLICT, 'CATEGORY_NAME_EXISTS', 'A category already uses this name.'); try { return await this.repository.updateCategory(id, input, actor.userId, actor.requestId); } catch (error) { this.translateConflict(error); } }
