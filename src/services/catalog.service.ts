...
import type { Actor, CategoryInput, CategoryUpdateInput, ImageInput, ImageUpdateInput, ProductInput, ProductQuery, ProductUpdateInput } from '../types/catalog';
...
export class CatalogService extends BaseService {
  ...
  public async addImage(productId: string, input: ImageInput, actor: Actor) { await this.requireManagedProduct(productId, actor); try { return await this.repository.addImage(productId, input, actor.userId, actor.requestId); } catch (error) { this.translateConflict(error); } }
  public async updateImage(productId: string, imageId: string, input: ImageUpdateInput, actor: Actor) { await this.requireManagedProduct(productId, actor); const image = await this.repository.findImage(imageId); if (!image || image.productId !== productId) throw new ApiError(HTTP_STATUS.NOT_FOUND, 'IMAGE_NOT_FOUND', 'Product image not found.'); try { return await this.repository.updateImage(imageId, input, actor.userId, actor.requestId); } catch (error) { this.translateConflict(error); } }
  public async deleteImage(productId: string, imageId: string, actor: Actor): Promise<void> { await this.requireManagedProduct(productId, actor); const image = await this.repository.findImage(imageId); if (!image || image.productId !== productId) throw new ApiError(HTTP_STATUS.NOT_FOUND, 'IMAGE_NOT_FOUND', 'Product image not found.'); await this.repository.deleteImage(imageId, actor.userId, actor.requestId); }
