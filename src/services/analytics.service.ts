...
export class AnalyticsService extends BaseService {
  ...
  public farmer(query: AnalyticsQuery, actor: AnalyticsActor) { return this.report('farmer', query, actor, async periods => { const [current, previous] = await Promise.all([this.repository.farmer(actor.userId, periods.current, true), this.repository.farmer(actor.userId, periods.previous, false)]); const { lowStockSummary, ...currentPeriod } = current; const { lowStockSummary: previousSnapshot, ...previousPeriod } = previous; void previousSnapshot; return { period: periodMetadata(periods), snapshot: { lowStockSummary }, current: currentPeriod, previous: previousPeriod, comparison: { revenue: moneyComparison(current.revenue, previous.revenue) } }; }); }
