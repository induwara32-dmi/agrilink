...
export class CommerceController extends BaseController {
  ...
  public readonly acceptFarmerOrder: RequestHandler = asyncHandler(async (request, response) => sendSuccess(response, HTTP_STATUS.OK, await this.service.acceptFarmerOrder(parameter(request, 'orderId'), parameter(request, 'farmerOrderId'), actorFrom(request))));
}
