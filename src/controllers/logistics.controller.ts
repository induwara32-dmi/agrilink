...
import type { AssignmentInput, DeliveryTransitionInput, LogisticsActor, ProofInput, ScheduleInput, VehicleInput, VehicleUpdateInput } from '../types/logistics';
import type { AcceptJobBody, CreateVehicleBody, ManualAssignmentBody, ProofBody, RejectJobBody, ScheduleDeliveryBody, TransitionDeliveryBody, UpdateVehicleBody } from '../validators/logistics.validators';
...
export class LogisticsController extends BaseController {
  ...
  public readonly addProof: RequestHandler = asyncHandler(async (request, response) => sendSuccess(response, HTTP_STATUS.OK, await this.service.addProof(parameter(request, 'deliveryId'), request.body as ProofBody as ProofInput, actorFrom(request))));
