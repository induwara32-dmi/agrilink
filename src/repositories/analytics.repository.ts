...
export interface ProductPerformanceRow { productId: string; productName: string; unit: string; revenue: Prisma.Decimal; quantity: Prisma.Decimal; orders: number }
...
export class AnalyticsRepository extends BaseRepository {
  ...
  public async farmer(userId: string, range: AnalyticsRange, includeSnapshot = true) {
    ...
    const [revenue, statuses, trends, topProducts, lowStock, turnover] = await Promise.all([
      ...
      this.database.$queryRaw<ProductPerformanceRow[]>(Prisma.sql`SELECT p.id AS "productId", p.name AS "productName", oi.unit, COALESCE(SUM(oi."lineTotal"), 0) AS revenue, COALESCE(SUM(oi.quantity), 0) AS quantity, COUNT(DISTINCT fo.id)::int AS orders FROM "OrderItem" oi JOIN "FarmerOrder" fo ON fo.id = oi."farmerOrderId" JOIN "Product" p ON p.id = oi."productId" WHERE fo."farmerId" = ${farmer.id}::uuid AND fo.status = 'DELIVERED'::"FarmerOrderStatus" AND fo."completedAt" >= ${range.from} AND fo."completedAt" < ${range.to} GROUP BY p.id, p.name, oi.unit ORDER BY revenue DESC LIMIT 10`),
    ...
    return { revenue: money(revenue, 'total'), salesTrends: trends, topProducts: topProducts.map(row => ({ ...row, revenue: row.revenue.toString(), quantity: row.quantity.toString() })), lowStockSummary: { count: lowStock.length, products: lowStock.map(row => ({ ...row, available: row.available.toString(), reorderLevel: row.reorderLevel.toString() })) }, inventoryTurnover: turnover.map(row => ({ ...row, soldQuantity: row.soldQuantity.toString(), averageInventory: row.averageInventory.toString(), turnover: row.turnover.toString() })), orderStatusDistribution: statuses.map(row => ({ status: row.status, count: row._count._all })) };
  }
  ...
  public async transporter(userId: string, range: AnalyticsRange) {
    ...
    const [earnings, completed, accepted, rejected, statuses, trends] = await Promise.all([
  ...
  public async admin(range: AnalyticsRange) {
    ...
    return { platformOverview: { newUsers: users, totalUsers, orders, totalOrders, products, totalProducts, categories, totalCategories, revenue: netRevenue }, userGrowth: { trend: userGrowth, byRole: userRoles.map(row => ({ role: row.role, count: row._count._all })) }, revenue: netRevenue, orders: { total: orders, cumulativeTotal: totalOrders, byStatus: orderStatuses.map(row => ({ status: row.status, count: row._count._all })) }, products: { total: products, cumulativeTotal: totalProducts, byStatus: productStatuses.map(row => ({ status: row.status, count: row._count._all })) }, categories: { total: categories, cumulativeTotal: totalCategories }, farmerPerformance: farmers.map(row => ({ ...row, revenue: row.revenue.toString() })), transporterPerformance: transporters.map(row => ({ ...row, earnings: row.earnings.toString() })) };
  }
