...
export const proofSchema = request(z.object({ proofStorageKey: z.string().trim().min(1).max(500).optional(), photoMetadata: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.null()])).optional(), receiverName: z.string().trim().min(2).max(180), receiverSignature: z.string().trim().max(20_000).optional(), notes: z.string().trim().max(2000).optional() }).refine(value => Boolean(value.proofStorageKey || value.receiverSignature), 'Photo or signature proof is required.'), deliveryParams, empty);
...
export type ProofBody = z.infer<typeof proofSchema>['body'];
